...
import type { View, Agent, CommandResponse, TaskPriority, Task } from '../../types';
...
const MyAiAvatar: React.FC<{ avatarUrl: string; isSpeaking: boolean }> = ({ avatarUrl, isSpeaking }) => (
    <div className="w-24 h-24 rounded-full bg-gradient-to-br from-cyan-400 to-fuchsia-500 p-1 flex-shrink-0 relative">
        ...
        {isSpeaking && <div className="absolute -top-1 -left-1 w-[104px] h-[104px] rounded-full border-2 border-cyan-400 animate-pulse"></div>}
    </div>
);
...
export const MyAiAssistant: React.FC<MyAiAssistantProps> = ({ setCurrentView }) => {
    const [prompt, setPrompt] = useState('');
    const [isLoading, setIsLoading] = useState(false);
    const [agents, setAgents] = useState<Agent[]>(initialAgents);
    const [aiMessage, setAiMessage] = useState("I am the Oasis assistant. Instruct your agents or teach a new workflow.");
    const [avatarUrl, setAvatarUrl] = useState('https://picsum.photos/seed/ai-avatar/100/100');
    ...
    const processCommand = useCallback(async (commandText: string) => {
        ...
        setIsLoading(true);
        setAgents(prev => prev.map(a => ({ ...a, status: 'working', task: { description: `Processing...`, priority: 'medium' } })));
        ...
        if (response.action === 'switch_view' && response.payload) {
            ...
            setAgents(prev => prev.map(a => ({ ...a, status: 'working', task: { description: response.message, priority: response.payload.priority } })));
        }
        ...
        workTimeoutRef.current = window.setTimeout(() => {
            setAgents(prev => prev.map(a => ({ ...a, status: 'idle', task: { description: 'Idle', priority: 'low' } })));
    ...
    return (
        <aside className="w-[380px] h-full bg-gray-900/50 border-l border-cyan-400/20 p-4 flex flex-col space-y-4">
            <GlassCard className="p-4 flex flex-col items-center text-center">
                <MyAiAvatar avatarUrl={avatarUrl} isSpeaking={isSpeaking} />
                <h2 className="text-xl font-bold mt-3 text-cyan-300">Assistant</h2>
                <p className="text-sm text-gray-300 min-h-[60px] mt-2">{typedAiMessage}</p>
            </GlassCard>

            <GlassCard className="flex-1 p-4 flex flex-col">
                 <h3 className="text-lg font-semibold text-fuchsia-400 mb-2">Cloud Employees</h3>
            ...
            <form onSubmit={handleFormSubmit} className="relative mt-auto">
                <input
                    ...
                    disabled={isLoading}
                    className="w-full bg-gray-800 border border-fuchsia-500/50 rounded-lg py-3 pl-12 pr-12 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-fuchsia-500 transition-all"
                />
                 <button type="button" onClick={isListening ? stopListening : startListening} className={`absolute left-2 top-1/2 -translate-y-1/2 p-2 rounded-md ${isListening ? 'text-red-500 animate-pulse' : 'text-cyan-400'} hover:bg-cyan-500/20`}>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>
                </button>
                <button type="submit" disabled={isLoading || !prompt} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-md text-fuchsia-400 hover:bg-fuchsia-500/20 disabled:text-gray-600">
                    {isLoading ? (
