    ...
    return (
        <div className={`flex items-start gap-4 ${isUser ? 'justify-end' : ''}`}>
            ...
            <div className={`max-w-xl p-4 rounded-lg ${isUser ? 'bg-blue-600' : 'bg-[#2a2f3b]'}`}>
                ...
                {message.fileName && (
                    <div className="mt-2 p-2 bg-blue-700/50 rounded-md flex items-center gap-2 border border-blue-500/50">
                        <FileIcon className="w-4 h-4 text-blue-300 flex-shrink-0" />
                        <span className="text-xs text-blue-200 font-mono truncate" title={message.fileName}>{message.fileName}</span>
                    </div>
                )}
